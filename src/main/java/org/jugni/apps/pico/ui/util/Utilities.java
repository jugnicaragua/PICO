...
/**
 * ************************************************************************* $name: Utilities.java
 * $version: 1.0 $date_modified: 210104 $description: $owner: Beans for Novell Services Team
...
public class Utilities {
  ...
  /*
   * public static boolean isEnumVirtual(ObjectEntryEnumeration enum) { boolean virtual = (enum
   * instanceof BrowserObjectEntryEnumeration);
   * 
   * if(virtual && (enum instanceof DefaultIndexedObjectEntryEnumeration)) { virtual = false; }
   * 
   * return virtual; }
   */
  /*
   * public static boolean checkForPageSnapins(ObjectEntryCollection entries) { // Only show the
   * Properties menu item if their are pages registered.
   * 
   * //REMIND: OK to pass null for MPEC parameter? InitSnapinInfo aInfo = new InitSnapinInfo(
   * MainShell.getInstance(), Shell.SNAPIN_PAGE, new PageSnapinContext(null, entries));
   * aInfo.setTracking( false ); boolean pagesExist =
   * Registry.checkSnapins(Scopes.page(entries.getFirstElement()), aInfo );
   * 
   * //Array pageSnapins = Registry.getSnapins(Scopes.page(entries.getFirstElement()), aInfo );
   * //if(pageSnapins != null && pageSnapins.size() > 0)
   * 
   * if(pagesExist) { return true; } else { return false; } }
   * 
   * public static ObjectEntry getMappedContainer(ObjectEntry oe) throws SnapinException { //REMIND:
   * Bug, we only return the first oe, what to do though? ObjectEntry[] oea =
   * ShellStubs.mapObjectEntry(oe);
   * 
   * ObjectEntry ret = oe;
   * 
  ...
  public static void showPopup(JPopupMenu p, Component c, int x, int y) {
    ...
    if (null != p && null != c) {
      ...
      if (null != sizePopup && null != sizeScreen) {
         ...
         * sizePopup.height > sizeScreen.height) ) ) { // center popup menu w/ submenus at center of
         * screen //x = (sizeScreen.width/2)-(pt.x)-sizePopup.width/2; //y =
         * (sizeScreen.height/2)-(pt.y)-sizePopup.height/2;
         * 
         * // keep it from extending off bottom of screen if (pt.y + y + sizePopup.height >
         * sizeScreen.height) { // move it up just enough to fit y -= (pt.y + y + sizePopup.height)
         * - sizeScreen.height + 5; } }
         * 
  ...
  public static void cp(ImageIcon imgIcon, String destino) throws IOException {
    ...
    if (image instanceof RenderedImage) {
      ...
      BufferedImage buffered = new BufferedImage(imgIcon.getIconWidth(), imgIcon.getIconHeight(),
