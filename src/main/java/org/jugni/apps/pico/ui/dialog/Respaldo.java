package org.jugni.apps.pico.ui.dialog;

import javax.swing.*;
import javax.swing.GroupLayout.Alignment;
import javax.swing.border.EmptyBorder;
import org.jugni.apps.pico.ui.util.JDialogoUtil;
...
/**
 * Formulario para respaldo se uiliza el grouplayout
 * 
 * @author gacs
 *
 */
public class Respaldo extends JDialogoUtil {
  ...
  private static final String ORINGENBD =
      System.getProperty("user.dir") + SEPARADOR + "var" + SEPARADOR + "bd" + SEPARADOR + "pico.db"; // direccion
                                                                                                     // origen
                                                                                                     // de
                                                                                                     // la
                                                                                                     // base
  ...
  private void initRespaldo() {
    String strDestino = System.getProperty("user.dir") + SEPARADOR + "var" + SEPARADOR + "respaldo"; // Direccion
                                                                                                     // destino
                                                                                                     // donde
                                                                                                     // se
                                                                                                     // almacenara
                                                                                                     // el
                                                                                                     // respaldo
    String nombreArchivo = "Respaldo"
        + new SimpleDateFormat("yyyy-mm-dd-HH:mm:ss").format(Calendar.getInstance().getTime()); // Nombre
                                                                                                // recomendado
                                                                                                // para
                                                                                                // el
                                                                                                // archivo
    ...
    // inicializa los paneles
    {
      ...
      hGroup.addGroup(gl.createParallelGroup().addComponent(lblDestino).addComponent(lblFile));
      hGroup.addGroup(gl.createParallelGroup()
          .addComponent(txtDestino, 240, GroupLayout.DEFAULT_SIZE, 241).addComponent(txtFile));
      hGroup.addComponent(cmdDestino);

      // Se agrega los componente al grupo vertical
      vGroup.addGroup(gl.createParallelGroup(Alignment.BASELINE).addComponent(lblDestino)
          .addComponent(txtDestino).addComponent(cmdDestino));
      vGroup.addGroup(
          gl.createParallelGroup(Alignment.BASELINE).addComponent(lblFile).addComponent(txtFile));
