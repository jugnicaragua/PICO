...
public class EmpresaForm extends JInternalFrame {
    ...
    private static final EmpresaForm INSTANCIA = new EmpresaForm();
    private static final String IMAGEN_PATH = "/jugnica.jpg";
    ...
    private void initEmpresa() {
        ...
        setClosable(true);
        Color bgCampoObligatorio = new Color(255, 228, 181); //Color de fondo para los campos que son obligatorios
        ...
        imgLogo = new ImageIcon(getClass().getResource(IMAGEN_PATH));
...
//        panel.setBounds(41, 51, 410, 204);
        ...
        pnlEmpresa.setLayout(new FormLayout(new ColumnSpec[]{
    ...
    private void showMessaje(String mensaje) {
        ...
        JOptionPane.showMessageDialog(null,
                strMensaje.toString(), getTitle(), JOptionPane.ERROR_MESSAGE);
    }
    ...
    /**
     * Rellena los campos del formulario, obteniendo los datos de la base de datos
     */
    private void rellenarCamposForm() {
        datosEmpresa = PicoCoreApplication.getBean(EmpresaService.class).buscar().orElse(new MiEmpresa());
    ...
    private void guardar() {
        if (loadEmpresa()) {
            try {
                Utilities.cp(imgLogo, IMAGEN_PATH);
            } catch (IOException ex) {
                Logger.getLogger(EmpresaForm.class.getName()).log(Level.SEVERE, null, ex);
            }
            PicoCoreApplication.getBean(EmpresaService.class).actualizar(datosEmpresa);
            dispose();
        }
    }

    public static EmpresaForm getInstancia() {
        return INSTANCIA;
    }
}
